...
def glcm_base(X, levels=16, direction=1, window_size=7, glb_mi=None, glb_ma=None):
    X_min = torch.min(X) if glb_mi is None else glb_mi
    X_max = torch.max(X) if glb_ma is None else glb_ma
    hw = window_size // 2
...
def glcm_energy(X, levels=16, direction=1, glb_mi=None, glb_ma=None):
    results = torch.sqrt(glcm_asm(X, levels, direction, glb_mi=glb_mi, glb_ma=glb_ma))
    return results
