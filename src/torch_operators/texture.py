...
def glcm_kernel(base, neighbour, levels):
    n = base.shape[0]
    level = torch.arange(n)
    result = base.new_zeros((n, levels, levels))
    result[level, base, neighbour] = 1
    result[level, neighbour, base] += 1
...
def glcm_compute_D0(window, levels):  # EAST
    base = window[:, :-1].flatten().long()
    neighbour = window[:, 1:].flatten().long()
...
def glcm_compute_D2(window, levels):  # SOUTH_EAST
    base = window[:-1, :-1].flatten().long()
    neighbour = window[1:, 1:].flatten().long()
    return glcm_kernel(base, neighbour, levels)
...
def glcm_base(X, levels=16, direction=1, window_size=7):
    ...
    if direction == 0:
        f = torch.vmap(glcm_compute_D0, in_dims=0, chunk_size=4096)
    elif direction == 1:
        f = torch.vmap(glcm_compute_D1, in_dims=0, chunk_size=4096)
    elif direction == 2:
        f = torch.vmap(glcm_compute_D2, in_dims=0, chunk_size=4096)
    elif direction == 3:
        f = torch.vmap(glcm_compute_D3, in_dims=0, chunk_size=4096)
...
def glcm_dissimilarity(X, levels=16, direction=1):
    ...
    I, J = torch.meshgrid(torch.arange(16), torch.arange(16), indexing="ij")
...
def glcm_correlation(X, levels=16, direction=1):
    glcm = glcm_base(X, levels, direction)
    J, I = torch.meshgrid(torch.arange(16), torch.arange(16), indexing="ij")
...
def glcm_mean(X, levels=16, direction=1):
    glcm = glcm_base(X, levels, direction)
    I = torch.arange(levels).reshape(1, 1, 1, levels, 1)
...
def glcm_variance(X, levels=16, direction=1):
    glcm = glcm_base(X, levels, direction)
    I = torch.arange(levels).reshape(1, 1, 1, 1, levels)
    mean = torch.sum(I * glcm, dim=(-2, -1))
    mean = mean.reshape(*mean.shape, 1, 1)
